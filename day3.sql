...
-- q 5 CTE1: Customers above gender average basket_count 
-- → label "Above Average"
-- CTE2: Customers below gender average basket_count → 
-- label "Below Average"
-- UNION both
-- Show customer_id, sex, basket_count, label


 with gender_average as (
  select *,
  avg(basket_count) over(partition by sex) as avgcount
  from (
	select cd.customer_id, cd.sex , bd.basket_count 
    from  customer_details as  cd
    join basket_details as bd   
    on cd.customer_id=bd.customer_id 
...
above_average as (
select * from gender_average
where basket_count >avgcount
),
below_average as (
select * from gender_average
where basket_count<avgcount
...
select customer_id, sex, basket_count , 'Above Average' as label
from above_average 
union 
select customer_id , sex, basket_count , 'Below Average' as label
from below_average;

-- Find male customers with total basket_count above overall average
-- UNION with
-- Female customers with total basket_count above their gender average
-- Show customer_id, sex, total

  with total as
  (select cd.customer_id, cd.sex ,sum(basket_count) as totalcount
from customer_details as cd
join basket_details as bd 
on cd.customer_id=bd.customer_id
group by customer_id, cd.sex),

totalavg as(
	select avg(totalcount)  as avgcount
    from total 
),

 
 female as(
 select avg(totalcount) as avg_total
 from total 
 where sex='female')
 
 select customer_id , sex , 'male over all'as label
 from total, totalavg
 where sex='Male'and totalcount>avgcount
 union
 select customer_id, sex , 'frmale above gender'as label 
 from total, female
 where totalcount>avg_total and sex='female';
 
 -- CTE1: Top 3 customers per gender by total basket_count
-- CTE2: Customers who never purchased
-- UNION both
-- Label each as "VIP" or "Inactive"
-- Show customer_id, sex, label

with total as(
select customer_id , sex , sum(basket_count) as totalcount from
(select distinct cd.customer_id, cd.sex , bd.basket_count 
from customer_details as cd
join basket_details as bd 
on cd.customer_id= bd.customer_id) as t
group by customer_id, sex) ,
male as(
...
select customer_id, sex ,'VIP' as label from male
union 
select customer_id, sex ,'VIP' as label from female
union
select customer_id, sex ,'Inacive' as label from neverpurchased;




-- views
-- Create a view called customer_summary that shows:
-- customer_id
-- sex
-- customer_age
-- total_basket_count
-- number_of_purchases
-- Then query it to find customers with more than 5 purchases!
 create view customer_summary as
select customer_id,sex, customer_age, count(distinct basket_date) as number_of_purchases , sum(basket_count) as total_basket_count from(
	select cd.customer_id,
    cd.sex,
    cd.customer_age,
    bd.basket_date,
    bd.basket_count
     ...
     join basket_details as bd 
     on cd.customer_id=bd.customer_id) as t 
     group by customer_id, sex , customer_age;
select * from customer_summary where number_of_purchases >4;

select min(basket_date) , max(basket_date) 
from basket_details;

select * from(
select customer_id, basket_date,
row_number() over(partition by basket_date, customer_id order by basket_date) as rn
from basket_details) t where rn>1;
...
select customer_id,sex, customer_age, count(distinct basket_date) as number_of_purchases , sum(basket_count) as total_basket_count from(
    ...
    bd.basket_count
     ...
     on cd.customer_id=bd.customer_id) as t 
     group by customer_id, sex , customer_age;
select * from summary where number_of_purchases >5;
